...
path = 'C:\Learning\Computer Science\Machine Learning\\03 My Projects\\01 Datasets\\02 NonBinaryClassification\kaggle_diagnosed_cbc_data_v4.csv'
...
class FFNN_NonBinaryClassification(nn.Module):
    def __init__(self):
        super().__init__()
...
def train(model, train_inputs, train_labels, test_inputs, test_labels, batch_size, num_epochs, criterion, optimizer):
    ...
    for epoch in range(num_epochs):
        ...
        # Compute the model's accuracy on test set and record the model's wigths corresponding to the best accuracy
        ...
        print(f"Epoch = {epoch + 1} ===> Loss = {total_loss: .3f} ===> Time = {epoch_time: .3f} ===> Test Accuracy = {test_accuracy: .4f} ===> Best Accuracy = {best_accuracy} amt epoch {best_epoch}")
...
ffnn = FFNN_NonBinaryClassification()
...
prediction_01 = ffnn(torch.FloatTensor(scaler.transform([[10, 43.2, 50.1, 4.3, 5, 2.77, 7.3, 24.2, 87.7, 26.3, 30.1, 189, 12.5, 0.17]])))
print("\nThe probabilities are: " + str(prediction_01))
print("The answer is: " + str((torch.argmax(prediction_01)).item() +1))
print("The true is: <index class = 6>")

prediction_02 = ffnn(torch.FloatTensor(scaler.transform([[7.2, 30.7, 60.7, 2.2, 4.4, 3.97, 9, 30.5, 77, 22.6, 29.5, 148, 14.3, 0.14]])))
print("\nThe probabilities are: " + str(prediction_02))
print("The answer is: " + str((torch.argmax(prediction_02)).item() + 1))
print("The true is: <index class = 2>")

prediction_03 = ffnn(torch.FloatTensor(scaler.transform([[5.2, 19.7, 72.4, 1, 3.8, 4.85, 13.2, 41, 84.7, 27.2, 32.1, 181, 10, 0.15]])))
print("\nThe probabilities are: " + str(prediction_03))
print("The answer is: " + str((torch.argmax(prediction_03)).item() +1))
print("The true is: <index class = 1>")
